...
@media print {
    ...
    .print-area { position: absolute; left:0; top:0; width:210mm; padding:15mm; background:white; border:none; box-shadow:none; }
...
.editable-field[contenteditable="false"] { background:#f3f4f6; }
.editable-field:focus {
    outline: none;
    box-shadow: 0 0 0 3px rgba(99,102,241,0.12);
    border-color: #6366f1;
}
.section-title { color:#1e40af; font-weight:700; font-size:15px; margin-bottom:6px; border-bottom:1px solid #cbd5e1; padding-bottom:6px; }
.table-xs th, .table-xs td { font-size:13px; padding:6px 8px; border:1px solid #cbd5e1; vertical-align: top; }
.section-box { border:1px solid #cbd5e1; padding:10px; border-radius:6px; background:#fff; }
...
.add-row-btn { background:#edf2ff; border:1px solid #c7d2fe; color:#3730a3; padding:4px 8px; border-radius:4px; cursor:pointer; font-size:12px; }
.remove-row-btn { background:#fff1f2; border:1px solid #fecaca; color:#991b1b; padding:4px 6px; border-radius:4px; cursor:pointer; font-size:12px; }
</style>

<div class="py-6 px-4 sm:px-6 lg:px-8 bg-gray-50 min-h-screen">
    <div class="print-area">
        <form id="applicationForm" method="POST" action="{{ route('applicant.application.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="grid grid-cols-4 gap-2 text-sm mb-4">
                ...
                <div>
                    ...
                    <select name="school_term" class="w-full border border-gray-400 px-2 py-0.5 text-[13px] rounded-sm">
                        <option value="1st Semester" {{ old('school_term')=='1st Semester' ? 'selected' : '' }}>1st Semester</option>
                        <option value="2nd Semester" {{ old('school_term')=='2nd Semester' ? 'selected' : '' }}>2nd Semester</option>
                        <option value="Trimester" {{ old('school_term')=='3rd Semester' ? 'selected' : '' }}>3rd Semester</option>
                    </select>
                </div>
            </div>
            ...
            <div class="section-box text-[13px] text-gray-800 leading-snug">
                <div class="grid grid-cols-4 gap-2 p-1.5">
                    <div>
                        ...
                        <div class="editable-field" contenteditable="false" data-field="last_name">{{ old('last_name', $user->last_name ?? '') }}</div>
                    </div>
                    <div>
                        <label class="block text-[12px] font-semibold">First Name</label>
                        <div class="editable-field" contenteditable="false" data-field="first_name">{{ old('first_name', $user->first_name ?? '') }}</div>
                    </div>
                    <div>
                        <label class="block text-[12px] font-semibold">Middle Name</label>
                        <div class="editable-field" contenteditable="false" data-field="middle_name">{{ old('middle_name', $user->middle_name ?? '') }}</div>
                    </div>
                    <div>
                        <label class="block text-[12px] font-semibold">Suffix</label>
                        <div class="editable-field" contenteditable="true" data-field="suffix">{{ old('suffix', $user->suffix ?? 'N/A') }}</div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-6 gap-2 p-1.5">
                    <div class="col-span-2">
                        ...
                        <div class="editable-field" contenteditable="true" data-field="address_no">{{ old('address_no', '') }}</div>
                    </div>
                    <div>
                        <label class="block text-[12px] font-semibold">Street</label>
                        <div class="editable-field" contenteditable="true" data-field="address_street">{{ old('address_street', '') }}</div>
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-6 gap-2 p-1.5">
                    ...
                    <div>
                        ...
                        <div class="editable-field" contenteditable="true" data-field="passport_no">{{ old('passport_no', '') }}</div>
                    </div>
                    <div class="col-span-2">
                        <label class="block text-[12px] font-semibold">E-mail Address</label>
                        <div class="editable-field" contenteditable="true" data-field="email_address">{{ old('email_address', $user->email ?? '') }}</div>
                    </div>
                </div>
                ...
                <div class="p-1.5">
                    ...
                    <div class="editable-field" contenteditable="true" data-field="current_address">{{ old('current_address', '') }}</div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-2 p-1.5">
                    <div>
                        ...
                        <div class="editable-field" contenteditable="true" data-field="telephone_nos">{{ old('telephone_nos', $user->phone ?? '') }}</div>
                    </div>
                    <div>
                        <label class="block text-[12px] font-semibold">Alternate Contact No.</label>
                        <div class="editable-field" contenteditable="true" data-field="alternate_contact">{{ old('alternate_contact', '') }}</div>
                    </div>
                </div>
        ...
        >
            ...
            <option value="Female" {{ old('sex') == 'Female' ? 'selected' : '' }}>Female</option>
                ...
                <div class="grid grid-cols-2 gap-2 p-1.5">
                    <div>
                        ...
                        <div class="editable-field" contenteditable="true" data-field="father_name">{{ old('father_name', '') }}</div>
                    </div>
                    <div>
                        <label class="block text-[12px] font-semibold">Mother’s Name</label>
                        <div class="editable-field" contenteditable="true" data-field="mother_name">{{ old('mother_name', '') }}</div>
                    </div>
                </div>
            </div>
            ...
            <table class="w-full border border-gray-400 text-[13px] text-gray-800 table-fixed mb-3">
                ...
                <tbody>
                    ...
                    <tr class="align-top">
                        <td class="border border-gray-400 font-semibold text-center py-1">{{ $label }}</td>

                        <td class="border border-gray-400 px-1">
                            <div class="editable-field min-h-[24px]" contenteditable="true" data-field="{{ $levelKey }}_period"></div>
                        </td>

                        <td class="border border-gray-400 px-1">
                            <div class="editable-field min-h-[24px]" contenteditable="true" data-field="{{ $levelKey }}_field"></div>
                        </td>

                        <td class="border border-gray-400 px-1">
                            <div class="editable-field min-h-[24px]" contenteditable="true" data-field="{{ $levelKey }}_university"></div>
                        </td>
                        ...
                        <td class="border border-gray-400 px-1 py-1">
                            <div class="grid grid-cols-2 text-[12px] leading-tight gap-x-2">
                                @if($label === 'BS')
                                    <label><input type="checkbox" name="bs_scholarship_type[]" value="PSHS" class="mr-1">PSHS</label>
                                    <label><input type="checkbox" name="bs_scholarship_type[]" value="RA 7687" class="mr-1">RA 7687</label>
                                    <label><input type="checkbox" name="bs_scholarship_type[]" value="MERIT" class="mr-1">MERIT</label>
                                    <label><input type="checkbox" name="bs_scholarship_type[]" value="RA 10612" class="mr-1">RA 10612</label>
                                @elseif($label === 'MS')
                                    <label><input type="checkbox" name="ms_scholarship_type[]" value="NSDB/NSTA" class="mr-1">NSDB/NSTA</label>
                                    <label><input type="checkbox" name="ms_scholarship_type[]" value="ASTHRDP" class="mr-1">ASTHRDP</label>
                                    <label><input type="checkbox" name="ms_scholarship_type[]" value="ERDT" class="mr-1">ERDT</label>
                                    <label><input type="checkbox" name="ms_scholarship_type[]" value="COUNCIL/SEI" class="mr-1">COUNCIL/SEI</label>
                                @elseif($label === 'PHD')
                                    <label><input type="checkbox" name="phd_scholarship_type[]" value="NSDB/NSTA" class="mr-1">NSDB/NSTA</label>
                                    <label><input type="checkbox" name="phd_scholarship_type[]" value="ASTHRDP" class="mr-1">ASTHRDP</label>
                                    <label><input type="checkbox" name="phd_scholarship_type[]" value="ERDT" class="mr-1">ERDT</label>
                                    <label><input type="checkbox" name="phd_scholarship_type[]" value="COUNCIL/SEI" class="mr-1">COUNCIL/SEI</label>
                                @endif
                            </div>
                            ...
                            <div class="text-[12px] mt-1 flex items-center gap-1">
                                ...
                                <div contenteditable="true"
                                    ...
                                    class="editable-field border-0 border-b border-gray-500 focus:outline-none focus:border-blue-500 w-36 inline-block min-h-[18px] px-1 bg-transparent"></div>
                            </div>
                        </td>

                        <td class="border border-gray-400 px-1">
                            <div class="editable-field min-h-[24px]" contenteditable="true" data-field="{{ $levelKey }}_remarks"></div>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
            ...
            <div class="border border-gray-400 rounded text-[13px] text-gray-800 leading-tight section-box p-0">
                ...
                <div class="px-3 py-2 text-[12px] text-justify border-b border-gray-400">
                    ...
                    <ol class="list-decimal list-inside space-y-1">
                        <li>An applicant for a graduate program should elect to go to another university if he/she earned his/her
                            1<sup>st</sup> (BS) and/or 2<sup>nd</sup> (MS) degrees from the same university to avoid inbreeding.</li>
                        <li>A faculty-applicant for a graduate program should elect to go to any of the member universities of the
                            ASTHRDP National Science Consortium, or the ERDT Consortium, or CBPSME National Consortium in Graduate
                            Science and Mathematics Education, or in a foreign university with good track record and/or recognized
                ...
                <table class="w-full text-[13px] border-t border-b border-gray-400 text-center">
                    <thead class="bg-gray-100 font-semibold">
                        <tr>
                            ...
                            <th class="border-r border-gray-400 py-1">TYPE OF APPLICANT<br><span class="font-normal text-xs">(for STRAND 2 only)</span></th>
                    ...
                    <tbody>
                        <tr>
                            <td class="border-r border-gray-400 align-top px-3 py-2 text-left">
                                <label><input type="checkbox" name="strand_category[]" value="STRAND 1" class="mr-1">STRAND 1</label><br>
                                <label><input type="checkbox" name="strand_category[]" value="STRAND 2" class="mr-1">STRAND 2</label>
                            </td>

                            <td class="border-r border-gray-400 align-top px-3 py-2 text-left">
                                <label><input type="checkbox" name="applicant_type[]" value="Student" class="mr-1">Student</label><br>
                                <label><input type="checkbox" name="applicant_type[]" value="Faculty" class="mr-1">Faculty</label>
                            </td>

                            <td class="align-top px-3 py-2 text-left">
                                <label><input type="checkbox" name="scholarship_type[]" value="MS" class="mr-1">MS</label><br>
                                <label><input type="checkbox" name="scholarship_type[]" value="PhD" class="mr-1">PhD</label>
                            </td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div class="px-3 py-2 border-t border-gray-400">
                    ...
                    <div class="mt-1 text-[13px] space-y-2">
                        <div class="flex items-center gap-2">
                            <span><strong>a.</strong> University where you applied/intend to enroll for graduate studies:</span>
                            <div contenteditable="true" data-field="new_applicant_university"
                                class="editable-field flex-1 border-0 border-b border-gray-400 bg-transparent min-h-[20px]"></div>
                        </div>
                        <div class="flex items-center gap-2">
                ...
                <div class="px-3 py-2 border-t border-gray-400 text-[13px]">
                    <div class="flex items-center gap-2">
                        ...
                        <label><input type="checkbox" name="research_approved" value="YES" class="ml-2 mr-1">YES</label>
                        <label><input type="checkbox" name="research_approved" value="NO" class="ml-2 mr-1">NO</label>
                    </div>

                    <div class="mt-2">
                        <span>Title:</span>
                        <div contenteditable="true" data-field="research_title"
                            class="editable-field border-0 border-b border-gray-400 bg-transparent inline-block w-[90%] min-h-[20px]"></div>
                    </div>

                    <div class="mt-2">
                        <span>Date of last enrollment in thesis/dissertation course:</span>
                        <div contenteditable="true" data-field="research_date"
                            class="editable-field border-0 border-b border-gray-400 bg-transparent inline-block w-[60%] min-h-[20px]"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="border border-gray-400 text-[13px] text-gray-800 leading-tight section-box">
                ...
                <div class="px-3 py-2">
                    ...
                    <div class="grid grid-cols-5 gap-3 mt-1 text-[13px]">
                        <label><input type="checkbox" name="employment_status[]" value="Permanent" class="mr-1">Permanent</label>
                        <label><input type="checkbox" name="employment_status[]" value="Contractual" class="mr-1">Contractual</label>
                        <label><input type="checkbox" name="employment_status[]" value="Probationary" class="mr-1">Probationary</label>
                        <label><input type="checkbox" name="employment_status[]" value="Self-employed" class="mr-1">Self-employed</label>
                        <label><input type="checkbox" name="employment_status[]" value="Unemployed" class="mr-1">Unemployed</label>
                    </div>
                </div>
                ...
                <div class="px-3 py-2">
                    ...
                    <div class="grid grid-cols-12 gap-x-2 gap-y-1 mt-1">
                        ...
                        <div class="col-span-4 border-b border-gray-400 editable-field" contenteditable="true" data-field="employed_position"></div>
                        <div class="col-span-3 text-right pr-1">Length of Service</div>
                        <div class="col-span-3 border-b border-gray-400 editable-field" contenteditable="true" data-field="employed_length_of_service"></div>

                        <div class="col-span-3 mt-1">Name of Company/Office</div>
                        <div class="col-span-9 mt-1 border-b border-gray-400 editable-field" contenteditable="true" data-field="employed_company_name"></div>

                        <div class="col-span-3 mt-1">Address of Company/Office</div>
                        <div class="col-span-9 mt-1 border-b border-gray-400 editable-field" contenteditable="true" data-field="employed_company_address"></div>

                        <div class="col-span-1 mt-1">Email</div>
                        <div class="col-span-5 mt-1 border-b border-gray-400 editable-field" contenteditable="true" data-field="employed_email"></div>
                        <div class="col-span-1 mt-1">Website</div>
                        <div class="col-span-5 mt-1 border-b border-gray-400 editable-field" contenteditable="true" data-field="employed_website"></div>

                        <div class="col-span-2 mt-1">Telephone No.</div>
                        <div class="col-span-4 mt-1 border-b border-gray-400 editable-field" contenteditable="true" data-field="employed_telephone"></div>
                        <div class="col-span-1 mt-1">Fax No.</div>
                        <div class="col-span-5 mt-1 border-b border-gray-400 editable-field" contenteditable="true" data-field="employed_fax"></div>
                    </div>
                </div>
                ...
                <div class="px-3 py-2">
                    ...
                    <div class="grid grid-cols-12 gap-x-2 gap-y-1 mt-1">
                        ...
                        <div class="col-span-10 border-b border-gray-400 editable-field" contenteditable="true" data-field="self_employed_business_name"></div>

                        <div class="col-span-2 mt-1">Address</div>
                        <div class="col-span-10 mt-1 border-b border-gray-400 editable-field" contenteditable="true" data-field="self_employed_address"></div>

                        <div class="col-span-2 mt-1">Email/Website</div>
                        <div class="col-span-3 mt-1 border-b border-gray-400 editable-field" contenteditable="true" data-field="self_employed_email_website"></div>

                        <div class="col-span-2 mt-1">Telephone No.</div>
                        <div class="col-span-3 mt-1 border-b border-gray-400 editable-field" contenteditable="true" data-field="self_employed_telephone"></div>

                        <div class="col-span-1 mt-1">Fax No.</div>
                        <div class="col-span-1 mt-1 border-b border-gray-400 editable-field" contenteditable="true" data-field="self_employed_fax"></div>

                        <div class="col-span-2 mt-1">Type of Business</div>
                        <div class="col-span-4 mt-1 border-b border-gray-400 editable-field" contenteditable="true" data-field="self_employed_type_of_business"></div>

                        <div class="col-span-2 mt-1">Years of Operation</div>
                        <div class="col-span-4 mt-1 border-b border-gray-400 editable-field" contenteditable="true" data-field="self_employed_years_of_operation"></div>
                    </div>
                </div>

                <!-- Scholarship Note -->
                <div class="px-3 py-2 mt-1 bg-gray-100 border-t border-gray-400 italic text-[12px]">
                    *Once accepted in the scholarship program, the scholar must obtain permission to go on a Leave of Absence (LOA)
                    from his/her employer and become a full-time student. The scholar must submit a letter from his/her employer
                    approving the LOA.
                </div>
            ...
            <div class="px-3 py-2 border-t border-gray-400">
                ...
                <div
                    class="editable-field w-full mt-1 border-b border-gray-400 overflow-hidden resize-none min-h-[1.5rem]"
                    contenteditable="true"
                    oninput="this.style.height='auto';this.style.height=this.scrollHeight+'px';"
                    data-field="research_plans"
                ></div>
            </div>
            ...
            <div class="section-box text-[13px] text-gray-800 leading-snug">
                ...
                <div class="p-1.5 border-b border-gray-300">
                    ...
                    <table class="w-full border border-gray-400 text-[13px] text-gray-800 mb-2">
                        ...
                        <tbody id="researchBody">
                            <tr>
                                <td class="table-xs"><div class="editable-field" contenteditable="true" data-field="research_involvements[0][field_title]"></div></td>
                                <td class="table-xs"><div class="editable-field" contenteditable="true" data-field="research_involvements[0][location_duration]"></div></td>
                                <td class="table-xs"><div class="editable-field" contenteditable="true" data-field="research_involvements[0][fund_source]"></div></td>
                                <td class="table-xs"><div class="editable-field" contenteditable="true" data-field="research_involvements[0][nature_of_involvement]"></div></td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="p-1.5 border-b border-gray-300">
                    ...
                    <table class="w-full border border-gray-400 text-[13px] text-gray-800 mb-2">
                        ...
                        <tbody id="pubBody">
                            <tr>
                                <td class="table-xs"><div class="editable-field" contenteditable="true" data-field="publications[0][title]"></div></td>
                                <td class="table-xs"><div class="editable-field" contenteditable="true" data-field="publications[0][name_year]"></div></td>
                                <td class="table-xs"><div class="editable-field" contenteditable="true" data-field="publications[0][nature_of_involvement]"></div></td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="p-1.5">
                    ...
                    <table class="w-full border border-gray-400 text-[13px] text-gray-800 mb-2">
                        ...
                        <tbody id="awardBody">
                            <tr>
                                <td class="table-xs"><div class="editable-field" contenteditable="true" data-field="awards[0][title]"></div></td>
                                <td class="table-xs"><div class="editable-field" contenteditable="true" data-field="awards[0][giving_body]"></div></td>
                                <td class="table-xs"><div class="editable-field" contenteditable="true" data-field="awards[0][year]"></div></td>
                            </tr>
                        </tbody>
                    </table>
            ...
            <div class="section-box mb-4 text-[13px] leading-snug text-gray-800">
                <p class="mb-2 font-semibold">Please attach clear scanned copies of the following documents (PDF format only, max 20 MB each):</p>
                ...
                <div class="space-y-2 pl-4">
                    ...
                    <div>
                        <p>• Endorsement 1 – Former professor in college for MS applicant / former professor in MS program for PhD applicant</p>
                        <input type="file" name="endorsement_1" accept="application/pdf"
                            class="block mt-1 w-full border border-gray-300 rounded-md p-1 text-sm text-gray-700">
                    </div>

                    <div>
                        <p>• Endorsement 2 – Former professor in college for MS applicant / former professor in MS program for PhD applicant</p>
                ...
                <div class="mt-3 pl-4">
                    ...
                    <div class="space-y-2 pl-4 mt-1">
                        ...
                        <div>
                            <p>• Form 2B – Certificate of DepEd Employment and Permit to Study (for DepEd employees only)</p>
            ...
            <div class="section-box mb-4 text-[13px] leading-snug">
                <p class="mb-2 text-justify">
                                    I hereby certify that all information given above are true and correct to the best of my knowledge. Any misinformation or withholding
                of information will automatically disqualify me from the program, Project Science and Technology Regional Alliance of Universities for National
                Development (STRAND). I am willing to refund all the financial benefits received plus appropriate interest if such misinformation is discovered.

                                </p>

                                <p class="mb-3 text-justify">
                                    Moreover, I hereby authorize the Science Education Institute of the Department of Science and Technology (SEI-DOST) to collect,
                record, organize, update or modify, retrieve, consult, use, consolidate, block, erase or destruct my personal data that I have provided in
                relation to my application to this scholarship. I hereby affirm my right to be informed, object to processing, access and rectify, suspend or
                withdraw my personal data, and be indemnified in case of damages pursuant to the provisions of the Republic Act No. 10173 of the Philippines,
                ...
                <div class="grid grid-cols-2 gap-6 mt-4">
                    <div>
                        ...
                        <div class="editable-field" contenteditable="true" data-field="applicant_name">{{ trim(($user->first_name ?? '') . ' ' . ($user->middle_name ?? '') . ' ' . ($user->last_name ?? '')) }}</div>
                    </div>
                    <div>
                    <label class="block text-[12px] font-semibold mb-1">Applicant Signature</label>
                    <canvas id="applicantSignature" class="border rounded w-full h-24" style="touch-action: none;"></canvas>
                    <button type="button" id="clearSignature" class="mt-2 px-2 py-1 text-sm bg-red-500 text-white rounded">Clear</button>
                    <input type="hidden" name="applicant_signature" id="applicantSignatureInput">
                </div>   
                <div class="mt-4 flex items-center gap-2">
                    <input type="checkbox" name="terms_and_conditions_agreed" required class="h-4 w-4 border-gray-400">
                    <span class="text-[12px]">I agree to the Terms, Conditions, and Data Privacy Policy.</span>
                </div>
            </div>

            <div class="text-right mt-8">
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2 rounded-md shadow-sm">Submit Application</button>
            </div>
        </form>
    </div>
</div>
