    ...
    <form id="filtersForm" method="GET" action="{{ route('admin.reports.monitoring') }}" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <select name="program" class="w-full rounded border-gray-300">
                    <option value="" {{ request('program') == '' ? 'selected' : '' }}>All Programs</option>
                    <option value="DOST" {{ request('program') == 'DOST' ? 'selected' : '' }}>DOST</option>
                    <option value="CHED" {{ request('program') == 'CHED' ? 'selected' : '' }}>CHED</option>
                </select>
            </div>
